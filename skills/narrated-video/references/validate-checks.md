...
that lack explicit `width:` and `height:` fields. Passes vacuously when the file does
not exist (it is generated by `nv sync` once the project has a flow scene with diagram
data defined in config).

Remedy: `declare width/height on every node — React Flow cannot measure a node in a headless render, and an unmeasured node renders 0×0 while the render still exits 0`

React Flow measures node dimensions with a `ResizeObserver` after first paint. A
headless Remotion renderer — separate browser tabs, each capturing one frame — never
fires that observer. Without explicit `width` and `height` on each node, React Flow
defaults to `0×0`, and every edge from that node converges on a point. The frame
still renders; the gate would still exit 0 without this check.

### CHK-36 — scene modules import React Flow only through the Diagram wrapper

Reads every `src/scenes/<Id>.tsx` the config declares and fails when any imports
directly from `@xyflow/react`.

Remedy: `use <Diagram> from kit components instead — direct @xyflow/react imports bypass the wrapper's determinism guards`

The `<Diagram>` component is where the determinism guards live: `fitView={false}`,
all interaction disabled, `defaultViewport` set explicitly. A scene that bypasses it
can re-enable `fitView` or leave `nodesDraggable` on — and the frame will render
